			...
			<div class="row">
				<div class="col-sm-8">
					...
					<div :class="{'form-group': true, 'has-error': form.errors.has('subcategory_name')}">
						...
						<div class="col-sm-8">
							<input v-model="form.subcategory_name" type="text" class="form-control" name="subcategory_name" id="subcategory_name" placeholder="Sub Category Name">
					...
					<div :class="{'form-group': true, 'has-error': form.errors.has('category_id')}">
						...
						<div class="col-sm-8">
							<select type="text" v-model="form.category_id" class="form-control" name="category_id" id="category_id">
							    ...
							    <option v-for="item in items" :value="item.id" v-text="item.category_name"></option>
							</select>		
...
<script>
	export default{
		...
		methods: {
			onSubmit() {
				this.form.post('/subcategory').then(status => this.status());
			},

			status() {
				flash('Sub Category added.');
			}
		}
	}
</script>